...
storageRouter.get("/interfaces/:interfaceId/history", async (request, response) => {
  ...
  const result = resolution === "raw"
    ? await pool.query(`WITH samples AS (SELECT collected_at,in_bps,out_bps,utilization_in_percent,utilization_out_percent,in_errors,out_errors,in_discards,out_discards,counter_reset,LAG(in_errors) OVER (ORDER BY collected_at) AS previous_in_errors,LAG(out_errors) OVER (ORDER BY collected_at) AS previous_out_errors,LAG(in_discards) OVER (ORDER BY collected_at) AS previous_in_discards,LAG(out_discards) OVER (ORDER BY collected_at) AS previous_out_discards FROM interface_samples WHERE interface_id=$1 AND collected_at>now()-make_interval(hours=>$2)) SELECT collected_at AS "timestamp",in_bps AS "inBps",out_bps AS "outBps",utilization_in_percent AS "utilizationInPercent",utilization_out_percent AS "utilizationOutPercent",CASE WHEN counter_reset OR previous_in_errors IS NULL THEN 0 ELSE GREATEST(0,COALESCE(in_errors,0)+COALESCE(out_errors,0)-COALESCE(previous_in_errors,0)-COALESCE(previous_out_errors,0)) END AS "errorDelta",CASE WHEN counter_reset OR previous_in_discards IS NULL THEN 0 ELSE GREATEST(0,COALESCE(in_discards,0)+COALESCE(out_discards,0)-COALESCE(previous_in_discards,0)-COALESCE(previous_out_discards,0)) END AS "discardDelta" FROM samples ORDER BY collected_at`, [request.params.interfaceId,hours])
    : await pool.query(`SELECT bucket_at AS "timestamp",in_bps_avg AS "inBps",in_bps_max AS "inBpsMax",out_bps_avg AS "outBps",out_bps_max AS "outBpsMax",utilization_in_avg AS "utilizationInPercent",utilization_out_avg AS "utilizationOutPercent",COALESCE(in_errors_delta,0)+COALESCE(out_errors_delta,0) AS "errorDelta",COALESCE(in_discards_delta,0)+COALESCE(out_discards_delta,0) AS "discardDelta" FROM interface_rollups WHERE interface_id=$1 AND resolution=$2 AND bucket_at>now()-make_interval(hours=>$3) ORDER BY bucket_at`, [request.params.interfaceId,resolution,hours]);
