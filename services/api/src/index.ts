...
async function queueAlertEvent(event:{triggerKind:string;deviceId:string;deviceName:string;checkId:string;checkName:string;checkKind:string;message:string}){
  ...
  for(const rule of rules.rows){const conditions=rule.conditions??{},deviceIds:string[]=conditions.deviceIds??[],checkKinds:string[]=conditions.checkKinds??[];if(deviceIds.length&&!deviceIds.includes(event.deviceId))continue;if(checkKinds.length&&!checkKinds.includes(event.checkKind))continue;const client=await pool.connect();try{await client.query("BEGIN");const title=`${event.deviceName} · ${event.checkName}`,occurrence=await client.query(`INSERT INTO alert_occurrences(rule_id,rule_name,trigger_kind,severity,device_id,check_id,title,message,context) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id`,[rule.id,rule.name,event.triggerKind,rule.severity,event.deviceId,event.checkId,title,event.message,event]);await client.query(`INSERT INTO alert_deliveries(occurrence_id,channel_id,channel_name,channel_kind) SELECT $1,c.id,c.name,c.kind FROM alert_channels c WHERE c.id=ANY($2::uuid[]) AND c.enabled=true AND (c.muted_until IS NULL OR c.muted_until<=now())`,[occurrence.rows[0].id,rule.channel_ids]);await client.query("UPDATE alert_rules SET last_triggered_at=now(),updated_at=now() WHERE id=$1",[rule.id]);await client.query("COMMIT");}catch(error){await client.query("ROLLBACK");throw error;}finally{client.release();}}
}
...
app.post("/api/alerts/rules",async(request,response)=>{if(!requireOperator(response))return;const parsed=alertRuleSchema.safeParse(request.body);if(!parsed.success)return response.status(400).json({error:"Complete the rule name, trigger and at least one channel"});const v=parsed.data,result=await pool.query(`INSERT INTO alert_rules(name,folder_id,trigger_kind,severity,conditions,channel_ids,cooldown_seconds,notify_recovery,enabled) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id`,[v.name,v.folderId,v.triggerKind,v.severity,{deviceIds:v.deviceIds,checkKinds:v.checkKinds},v.channelIds,v.cooldownSeconds,v.notifyRecovery,v.enabled]);response.status(201).json(result.rows[0]);});
