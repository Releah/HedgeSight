...
async function ssh(job:ProbeJob):Promise<ProbeResult>{
  ...
  try{
    ...
    const adapters=interfaces.split("\n").filter(Boolean).map(line=>{const [name,macAddress,mtu,state,speedMbps,inOctets="0",outOctets="0",inErrors="0",outErrors="0",inDiscards="0",outDiscards="0"]=line.split("|");const speed=Number(speedMbps)>0?Number(speedMbps):null,key=`${job.deviceId}:${name}`,current={at:now,inOctets:BigInt(inOctets||0),outOctets:BigInt(outOctets||0),errors:BigInt(inErrors||0)+BigInt(outErrors||0)+BigInt(inDiscards||0)+BigInt(outDiscards||0)},previous=sshCounterHistory.get(key);let inBps:number|null=null,outBps:number|null=null,utilizationPercent:number|null=null,errorDelta=0;if(previous&&now>previous.at&&current.inOctets>=previous.inOctets&&current.outOctets>=previous.outOctets){const seconds=(now-previous.at)/1000;inBps=Number(current.inOctets-previous.inOctets)*8/seconds;outBps=Number(current.outOctets-previous.outOctets)*8/seconds;utilizationPercent=speed?Math.max(inBps,outBps)/(speed*1_000_000)*100:null;errorDelta=Number(current.errors>=previous.errors?current.errors-previous.errors:0n);highestInterfaceUtilizationPercent=Math.max(highestInterfaceUtilizationPercent,utilizationPercent??0);interfaceErrorDelta+=errorDelta;}sshCounterHistory.set(key,current);return {name,macAddress,mtu:Number(mtu)||null,state,speedMbps:speed,inBps,outBps,utilizationPercent,errorDelta,counters:{inOctets,outOctets,inErrors,outErrors,inDiscards,outDiscards}};});
    const memoryBytes=Number(facts.mem_kb??0)*1024,memoryUsedPercent=Number(facts.mem_kb)?100*(1-Number(facts.mem_available_kb??0)/Number(facts.mem_kb)):0,cpuUsedPercent=Number(facts.cpu_used_percent??0),diskHighestUsedPercent=Math.max(0,...disks.map(item=>item.usedPercent));
    const limits={cpu:Number(job.config.cpuThresholdPercent??90),memory:Number(job.config.memoryThresholdPercent??90),disk:Number(job.config.diskThresholdPercent??90),interfaceUtilization:Number(job.config.interfaceThresholdPercent??90),interfaceErrors:Number(job.config.interfaceErrorThreshold??1)};const alerts:string[]=[];if(cpuUsedPercent>=limits.cpu)alerts.push(`CPU ${cpuUsedPercent.toFixed(1)}%`);if(memoryUsedPercent>=limits.memory)alerts.push(`memory ${memoryUsedPercent.toFixed(1)}%`);if(diskHighestUsedPercent>=limits.disk)alerts.push(`disk ${diskHighestUsedPercent.toFixed(1)}%`);if(highestInterfaceUtilizationPercent>=limits.interfaceUtilization)alerts.push(`interface ${highestInterfaceUtilizationPercent.toFixed(1)}%`);if(interfaceErrorDelta>=limits.interfaceErrors&&limits.interfaceErrors>0)alerts.push(`${interfaceErrorDelta} interface errors/discards`);
    return {status:alerts.length?"degraded":"up",startedAt:startedAt.toISOString(),finishedAt:new Date().toISOString(),latencyMs:performance.now()-start,message:alerts.length?`Resource threshold exceeded: ${alerts.join(", ")}`:`SSH profile collected from ${hostname}`,metrics:{sshResponseTimeMs:performance.now()-start,cpuUsedPercent,memoryUsedPercent,memoryBytes,diskHighestUsedPercent,highestInterfaceUtilizationPercent,interfaceErrorDelta},observations:{hostname,osName:os.PRETTY_NAME??os.NAME??"Linux",osVersion:os.VERSION_ID??"",kernel:facts.kernel??"",cpuCount:Number(facts.cpu_count??0),cpuUsedPercent,memoryBytes,memoryUsedPercent,uptimeSeconds:Number(facts.uptime_seconds??0),filesystems:disks,interfaces:adapters,hostKeyFingerprint:fingerprint}};
