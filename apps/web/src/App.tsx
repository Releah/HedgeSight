...
type AlertRule={id:string;name:string;folderId:string|null;triggerKind:string;severity:"info"|"warning"|"critical";messageTemplate:string;conditions:{deviceIds?:string[];checkKinds?:string[]};channelIds:string[];audienceGroupIds:string[];enabled:boolean;mutedUntil:string|null;cooldownSeconds:number;notifyRecovery:boolean;lastTriggeredAt:string|null};
type NotificationGroup={id:string;name:string;description:string;enabled:boolean;userIds:string[];channelIds:string[]};
type AlertUser={id:string;displayName:string;email:string;role:string};
...
type AlertData={folders:AlertFolder[];channels:AlertChannel[];groups:NotificationGroup[];users:AlertUser[];rules:AlertRule[];deliveries:AlertDelivery[];summary:{enabledRules:number;enabledChannels:number;failedDeliveries:number;deliveredToday:number}};
...
function AlertsPage({devices,canEdit,canAdmin}:{devices:MonitoringDevice[];canEdit:boolean;canAdmin:boolean}){
  const [data,setData]=useState<AlertData|null>(null),[tab,setTab]=useState<"overview"|"rules"|"audiences"|"channels"|"history">("overview"),[message,setMessage]=useState(""),[ruleDialog,setRuleDialog]=useState(false);
  ...
  async function createRule(event:FormEvent<HTMLFormElement>){event.preventDefault();const d=new FormData(event.currentTarget);const ok=await request("/api/alerts/rules",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:d.get("name"),folderId:d.get("folderId")||null,triggerKind:d.get("triggerKind"),severity:d.get("severity"),messageTemplate:d.get("messageTemplate"),deviceIds:d.getAll("deviceIds"),checkKinds:d.getAll("checkKinds"),channelIds:d.getAll("channelIds"),audienceGroupIds:d.getAll("audienceGroupIds"),cooldownSeconds:Number(d.get("cooldownSeconds")),notifyRecovery:d.get("notifyRecovery")==="on",enabled:true})});if(ok)setRuleDialog(false);}
  async function createGroup(event:FormEvent<HTMLFormElement>){event.preventDefault();const form=event.currentTarget,d=new FormData(form);if(await request("/api/alerts/groups",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name:d.get("name"),description:d.get("description"),userIds:d.getAll("userIds"),channelIds:d.getAll("channelIds")})}))form.reset();}
  ...
  const ruleCard=(rule:AlertRule)=><article className={`alert-rule-card ${rule.enabled?"":"disabled"}`} key={rule.id} draggable={canEdit} onDragStart={event=>event.dataTransfer.setData("text/alert-rule",rule.id)}><div><span className={`alert-severity ${rule.severity}`}>{rule.severity}</span><strong>{rule.name}</strong></div><p>{rule.triggerKind.replaceAll("_"," ")} · {rule.conditions?.deviceIds?.length||"all"} devices · {rule.channelIds.length} channel{rule.channelIds.length===1?"":"s"}</p><code>{rule.messageTemplate}</code><small>{rule.lastTriggeredAt?`Last triggered ${relativeTime(rule.lastTriggeredAt)}`:"Never triggered"} · cooldown {Math.round(rule.cooldownSeconds/60)}m</small>{canEdit&&<footer><button className="secondary" onClick={()=>void request(`/api/alerts/rules/${rule.id}/test`,{method:"POST"})}><Send size={12}/>Test alert</button><button className="secondary" onClick={()=>void request(`/api/alerts/rules/${rule.id}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({enabled:!rule.enabled})})}>{rule.enabled?"Disable":"Enable"}</button><button className="secondary" onClick={()=>void request(`/api/alerts/rules/${rule.id}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({mutedUntil:rule.mutedUntil?null:new Date(Date.now()+3600000).toISOString()})})}><VolumeX size={12}/>{rule.mutedUntil?"Unmute":"Mute 1h"}</button><button className="icon-button danger" onClick={()=>void request(`/api/alerts/rules/${rule.id}`,{method:"DELETE"})}><Trash2 size={13}/></button></footer>}</article>;
  return <section className="alerts-page">
    <div className="alerts-tabs">{(["overview","rules","audiences","channels","history"] as const).map(item=><button key={item} className={tab===item?"active":""} onClick={()=>setTab(item)}>{item==="history"?"Delivery history":item}</button>)}</div>
    {tab==="audiences"&&<><section className="alert-channel-grid">{data.groups.map(group=><article key={group.id}><span className="channel-kind"><Users/></span><div><strong>{group.name}</strong><small>{group.userIds.length} users · {group.channelIds.length} channels</small><p>{group.description||"No description"}</p></div>{canAdmin&&<footer><button className="secondary" onClick={()=>void request(`/api/alerts/groups/${group.id}`,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({...group,enabled:!group.enabled})})}>{group.enabled?"Disable":"Enable"}</button><button className="icon-button danger" onClick={()=>void request(`/api/alerts/groups/${group.id}`,{method:"DELETE"})}><Trash2 size={13}/></button></footer>}</article>)}</section>{canAdmin&&<Panel title="Create notification audience" subtitle="Audience membership is independent from application permissions" className="full-panel"><form className="notification-group-form" onSubmit={createGroup}><label>Name<input name="name" required placeholder="Out-of-hours network team"/></label><label>Description<input name="description" placeholder="People responsible for OOH infrastructure events"/></label><fieldset><legend>Members</legend>{data.users.map(member=><label key={member.id}><input type="checkbox" name="userIds" value={member.id}/><span>{member.displayName}<small>{member.email} · {member.role}</small></span></label>)}</fieldset><fieldset><legend>Delivery channels</legend>{data.channels.filter(channel=>channel.enabled).map(channel=><label key={channel.id}><input type="checkbox" name="channelIds" value={channel.id}/>{channel.name} <small>{channel.kind}</small></label>)}</fieldset><button className="primary"><Users size={14}/>Create audience</button></form></Panel>}</>}
    {tab==="audiences"&&canEdit&&<Panel title="Standard workflow notification" subtitle="Connect incidents, major incidents, maintenance and tasks to an audience" className="full-panel"><form className="workflow-policy-form" onSubmit={createRule}><label>Policy name<input name="name" required placeholder="Notify OOH team when an incident opens"/></label><label>When<select name="triggerKind"><option value="incident_created">Incident created</option><option value="incident_resolved">Incident resolved</option><option value="major_incident_created">Major incident declared</option><option value="major_incident_resolved">Major incident resolved</option><option value="maintenance_created">Maintenance scheduled</option><option value="maintenance_started">Maintenance started</option><option value="maintenance_ended">Maintenance ended</option><option value="maintenance_overrun">Maintenance overrun</option><option value="task_created">Task created</option><option value="task_assigned">Task assigned</option><option value="task_completed">Task completed</option></select></label><label>Audience<select name="audienceGroupIds" required><option value="">Choose audience</option>{data.groups.filter(group=>group.enabled).map(group=><option key={group.id} value={group.id}>{group.name}</option>)}</select></label><label>Severity<select name="severity"><option value="info">Information</option><option value="warning">Warning</option><option value="critical">Critical</option></select></label><label className="wide">Message template<textarea name="messageTemplate" required defaultValue="$SEVERITY: $EVENT for $NODE at $TIME. $MESSAGE"/><small>$EVENT, $NODE, $REFERENCE, $ACTOR, $TIME, $STATUS, $SEVERITY and $MESSAGE are supported.</small></label><input type="hidden" name="cooldownSeconds" value="0"/><button className="primary"><BellRing size={14}/>Create notification policy</button></form></Panel>}
